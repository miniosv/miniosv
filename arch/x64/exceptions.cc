...
interrupt_descriptor_table idt
    __attribute__((init_priority((int)init_prio::idt)));
...
interrupt_descriptor_table::interrupt_descriptor_table() {
  ...
  for (unsigned i = 32; i < 256; ++i) {
    add_entry(i, 2,
              reinterpret_cast<void (*)()>(interrupt_entry + (i - 32) * 16));
  }
}

void interrupt_descriptor_table::add_entry(unsigned vec, unsigned ist,
                                           void (*handler)()) {
...
void interrupt_descriptor_table::load_on_cpu() {
  processor::desc_ptr d(sizeof(_idt) - 1, reinterpret_cast<ulong>(&_idt));
  processor::lidt(d);
}

unsigned interrupt_descriptor_table::register_interrupt_handler(
    std::function<bool()> pre_eoi, std::function<void()> eoi,
...
shared_vector interrupt_descriptor_table::register_level_triggered_handler(
    unsigned gsi, std::function<bool()> pre_eoi,
    std::function<void()> post_eoi) {
  WITH_LOCK(_lock) {
    for (unsigned i = 32; i < 256; ++i) {
      auto o = _handlers[i].read_by_owner();
      if ((o && o->gsi == gsi) || o == nullptr) {
        auto n =
            new handler(o, pre_eoi, [] { processor::apic->eoi(); }, post_eoi);
        n->gsi = gsi;
...
void interrupt_descriptor_table::unregister_level_triggered_handler(
    shared_vector v) {
...
unsigned
interrupt_descriptor_table::register_handler(std::function<void()> post_eoi) {
  return register_interrupt_handler([] { return true; },
                                    [] { processor::apic->eoi(); }, post_eoi);
}

void interrupt_descriptor_table::register_interrupt(
    inter_processor_interrupt *interrupt) {
  unsigned v = register_handler(interrupt->get_handler());
  interrupt->set_vector(v);
}

void interrupt_descriptor_table::unregister_interrupt(
    inter_processor_interrupt *interrupt) {
  unregister_handler(interrupt->get_vector());
}

void interrupt_descriptor_table::register_interrupt(
    gsi_edge_interrupt *interrupt) {
  unsigned v = register_handler(interrupt->get_handler());
  interrupt->set_vector(v);
  interrupt->enable();
}
...
void interrupt_descriptor_table::register_interrupt(
    gsi_level_interrupt *interrupt) {
  shared_vector v = register_level_triggered_handler(
      interrupt->get_id(), interrupt->get_ack(), interrupt->get_handler());
  interrupt->set_vector(v);
  interrupt->enable();
}
...
extern "C" void general_protection(exception_frame *ef) {
...
#define DUMMY_HANDLER(x)                                                       \
  extern "C" void x(exception_frame *ef);                                      \
  void x(exception_frame *ef) {                                                \
    dump_registers(ef);                                                        \
    abort("DUMMY_HANDLER for " #x " aborting.\n");                             \
  }
